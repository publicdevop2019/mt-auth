...
public class Analytics {
    ...
    public static void stopSinglePublish(Long startAt, StoredEvent event, Long sequenceId) {
        long stopAt = Instant.now().toEpochMilli();
        long elapse = stopAt - startAt;
        if (elapse > Type.EVENT_SINGLE_PUBLISH_CONFIRM.getBudget()) {
            if (event == null) {
                log.error(
                    "{} exceed budget, took {} event detail unknown, sequence id {} start at {}",
                    Type.EVENT_BATCH_PUBLISH_CONFIRM.getLabel(),
                    prettyTime(elapse), sequenceId, startAt);
            } else {
                String traceId = event.getTraceId();
                MDC.put(TRACE_ID_LOG, traceId);
                log.warn("{} exceed budget, took {} event id {} name {}",
                    Type.EVENT_SINGLE_PUBLISH_CONFIRM.getLabel(),
    ...
    public static void stopBatchPublish(Long startAt, StoredEvent event, Long sequenceId) {
        long stopAt = Instant.now().toEpochMilli();
        long elapse = stopAt - startAt;
        if (elapse > Type.EVENT_BATCH_PUBLISH_CONFIRM.getBudget()) {
            if (event == null) {
                log.error(
                    "{} exceed budget, took {} event detail unknown, sequence id {} start at {}",
                    Type.EVENT_BATCH_PUBLISH_CONFIRM.getLabel(),
    ...
    public enum Type {
        ...
        DATA_QUERY_ALL("data_query_all", 5000L),
