...
import { INewRole } from 'src/app/pages/my-roles/my-roles.component';
...
export class RoleComponent extends Aggregate<RoleComponent, INewRole> implements OnInit, AfterViewInit, OnDestroy {
  ...
  public loadRoot = this.permissoinSvc.readByQuery(0, 1000, "parentId:null")
  ...
  ngAfterViewInit(): void {
    if (this.bottomSheet.context === 'edit') {
      ...
      this.aggregate.permissions.forEach(p => {
      ...
      this.fis.formGroupCollection[this.formId].get('permissions').setValue(this.aggregate.permissions)
  ...
  convertToPayload(cmpt: RoleComponent): INewRole {
    ...
    return {
      ...
      permissions: Object.keys(value).filter(e => value[e] === 'checked'),
