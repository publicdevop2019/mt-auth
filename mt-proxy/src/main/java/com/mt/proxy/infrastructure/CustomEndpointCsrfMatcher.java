...
import com.mt.proxy.infrastructure.spring_cloud_gateway.ScgEndpointFilter;
        ...
        return Mono.just(exchange.getRequest())
            .filter(request -> {
                if (ScgEndpointFilter.isWebSocket(request.getHeaders())) {
                    ...
                    Optional<MethodPathKey> first = patchKeyStream.stream().filter(
                        pattern -> matcher.match(pattern.getPath(), request.getPath().value()))
                        .filter(key -> key.getMethod().equalsIgnoreCase(request.getMethodValue()))
                        .findFirst();
                    if (first.isPresent()) {
