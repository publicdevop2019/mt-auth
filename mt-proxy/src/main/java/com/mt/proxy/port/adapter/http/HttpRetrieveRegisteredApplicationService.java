...
public class HttpRetrieveRegisteredApplicationService
    ...
    public Set<RegisteredApplication> fetchAll() {
        ...
        if (eurekaClient.getApplication(appName) != null) {
            ...
            ResponseEntity<SumPagedRep<RegisteredApplication>> exchange = restTemplate
                .exchange(url + "?page=size:40,num:0", HttpMethod.GET, null,
            ...
            if (exchange.getStatusCode().is2xxSuccessful()) {
                ...
                if (body == null) {
                    throw new IllegalStateException(
                        "unable to load registered application from remote: " + url);
                }
                if (body.getData().size() == 0) {
                    throw new IllegalStateException(
                        "registered application from remote: " + url + " is empty");
                }
                ...
                for (int a = 1; a < i; a++) {
                    ResponseEntity<SumPagedRep<RegisteredApplication>> exchange2 = restTemplate
                        .exchange(url + "?page=size:40,num:" + a, HttpMethod.GET, null,
                            new ParameterizedTypeReference<>() {
                            });
                    SumPagedRep<RegisteredApplication> body2 = exchange2.getBody();
                    if (body2 == null || body2.getData().size() == 0) {
                        throw new IllegalStateException(
                            "unable to load registered application from remote");
                    }
                ...
                throw new IllegalStateException(
                    "error during load endpoint profile, check log for more details");
            }
        } else {
            log.error("reload request was ignore due to service is not ready");
            throw new IllegalStateException(
                "reload request was ignore due to service is not ready");
        }
        return data;
    }
