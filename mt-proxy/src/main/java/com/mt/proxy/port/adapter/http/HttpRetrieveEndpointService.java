...
public class HttpRetrieveEndpointService implements RetrieveEndpointService {
    ...
    public Set<Endpoint> loadAllEndpoints() {
        ...
        if (eurekaClient.getApplication(appName) != null) {
            ...
            ResponseEntity<SumPagedRep<Endpoint>> exchange = restTemplate
            ...
            if (exchange.getStatusCode().is2xxSuccessful()) {
                ...
                if (body == null) {
                    throw new IllegalStateException(
                        "unable to load endpoint profile from remote: " + url);
                }
                if (body.getData().size() == 0) {
                    throw new IllegalStateException(
                        "endpoint profile from remote: " + url + " is empty");
                }
                ...
                for (int a = 1; a < i; a++) {
                    ResponseEntity<SumPagedRep<Endpoint>> exchange2 = restTemplate
                    ...
                    if (body2 == null || body2.getData().size() == 0) {
                        throw new IllegalStateException(
                            "unable to load endpoint profile from remote");
                    }
