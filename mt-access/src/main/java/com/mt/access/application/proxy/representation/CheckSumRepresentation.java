...
public class CheckSumRepresentation {
    ...
    public CheckSumRepresentation(CheckSumValue hostCheckSum, Map<ProxyInfo, CheckSumValue> cacheEndpointSum) {
        hostValue=hostCheckSum.getValue();
        proxyValue=new HashMap<>();
        cacheEndpointSum.forEach((k,v)->{
            proxyValue.put(k.getId(),v.getValue());
        });
