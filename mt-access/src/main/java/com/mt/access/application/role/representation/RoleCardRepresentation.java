...
public class RoleCardRepresentation {
    ...
    public static SumPagedRep<RoleCardRepresentation> updateName(
        ...
        Set<ProjectId> collect =
            data.stream().filter(e -> e.roleType.equals(RoleType.PROJECT)).flatMap(e -> {
                if (e.tenantId != null) {
        ...
        Set<ProjectId> collect2 =
            data.stream().filter(e -> e.tenantId != null).map(e -> new ProjectId(e.tenantId))
                .collect(Collectors.toSet());
        ...
        if (collect.size() > 0) {
            ...
            data.forEach(e -> {
                ...
                if (e.tenantId != null) {
                    allByQuery.stream()
                        .filter(ee2 -> ee2.getProjectId().getDomainId().equals(e.tenantId))
                        .findFirst().ifPresent(eee -> e.tenantId = eee.getName());
                }
            });
        }
        Set<ClientId> collect1 = data.stream().filter(e -> e.roleType.equals(RoleType.CLIENT))
        ...
        if (collect1.size() > 0) {
            ...
            data.forEach(e -> {
                if (e.roleType.equals(RoleType.CLIENT)) {
                    allByQuery2.stream().filter(ee -> ee.getClientId().getDomainId().equals(e.name))
                        .findFirst().ifPresent(ee -> e.name = ee.getName());
                }
            });
        }
