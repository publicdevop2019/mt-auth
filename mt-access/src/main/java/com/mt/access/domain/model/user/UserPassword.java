...
public class UserPassword {
    ...
    private void setPassword(String rawPassword) {
        ...
        if (!m.find()) {
            throw new DefinedRuntimeException("at least one letter", "0004",
        ...
        if (!m2.find()) {
            throw new DefinedRuntimeException("at least one number", "0004",
                HttpResponseCode.BAD_REQUEST,
                ExceptionCatalog.ILLEGAL_ARGUMENT);
        }
        ...
        if (m3.find()) {
            throw new DefinedRuntimeException("no space allowed", "0004",
                HttpResponseCode.BAD_REQUEST,
                ExceptionCatalog.ILLEGAL_ARGUMENT);
        }
        ...
        if (!m4.find()) {
            throw new DefinedRuntimeException("at least one special character", "0004",
                HttpResponseCode.BAD_REQUEST,
                ExceptionCatalog.ILLEGAL_ARGUMENT);
        }
